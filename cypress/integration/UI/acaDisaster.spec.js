...
context('UI',() => {
    ...
    describe('One VIN - ACA UI - DISASTER',() => {
        beforeEach(() => {
            cy.logInUI({email: Cypress.env('username'), password: Cypress.env('password')})
            cy.selectACA()
        })
        it('requests a single vin disaster',() => {
            cy.get('[name="VIN"]').type('JH4DC4433RS801008').blur()
            cy.get('[role="button"]').click()
            cy.get('[role="listbox"]').should('be.visible')
            cy.wait(500)
            cy.get('[role="option"]').contains('Disaster').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'Disaster')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@acaDisaster')
            getIframeBody().find('p').contains('Disaster Report').should('be.visible')
