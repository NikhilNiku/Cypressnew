...
context('UI',() => {
    ...
    describe('One VIN - ACA UI - RECALL',() => {
        beforeEach(() => {
            cy.logInUI({email: Cypress.env('username'), password: Cypress.env('password')})
            cy.selectACA()
        })
        it('requests a single vin recall',() => {
            cy.get('[name="VIN"]').type('JH4DC4433RS801008').blur()
            cy.get('[role="button"]').click()
            cy.get('[role="listbox"]').should('be.visible')
            cy.wait(500)
            cy.get('[role="option"]').contains('Recall').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'Recall')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@acaRecall')
            getIframeBody().find('p').contains('Recall Report').should('be.visible')
