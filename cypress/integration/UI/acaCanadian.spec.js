...
context('UI',() => {
    ...
    describe('One VIN - ACA UI - CANADIAN',() => {
        beforeEach(() => {
            cy.logInUI({email: Cypress.env('username'), password: Cypress.env('password')})
            cy.selectACA()
        })
        it('requests a single vin canadian',() => {
            cy.get('[name="VIN"]').type('JH4DC4433RS801008').blur()
            cy.get('[role="button"]').click()
            cy.get('[role="listbox"]').should('be.visible')
            cy.wait(500)
            cy.get('[role="option"]').contains('Canadian').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'Canadian')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@acaCanadian')
            getIframeBody().find('p').contains('Canadian Report').should('be.visible')
