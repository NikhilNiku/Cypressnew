...
context('UI',() => {
    ...
    describe('One VIN - ACA UI - DISCREPANCY',() => {
        beforeEach(() => {
            cy.logInUI({email: Cypress.env('username'), password: Cypress.env('password')})
            cy.selectACA()
        })
        it('requests a single vin discrepancy',() => {
            cy.get('[name="VIN"]').type('JH4DC4433RS801008').blur()
            cy.get('[role="button"]').click()
            cy.get('[role="listbox"]').should('be.visible')
            cy.wait(500)
            cy.get('[role="option"]').contains('Discrepancy').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'Discrepancy')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@acaDiscrepancy')
            getIframeBody().find('p').contains('Mileage Discrepancy Report').should('be.visible')
