...
context('UI',() => {
    ...
    describe('One VIN - ACA UI - HAA',() => {
        beforeEach(() => {
            cy.logInUI({email: Cypress.env('username'), password: Cypress.env('password')})
            cy.selectACA()
        })
        it('requests a single vin haa',() => {
            cy.get('[name="VIN"]').type('JH4DC4433RS801008').blur()
            cy.get('[role="button"]').click()
            cy.get('[role="listbox"]').should('be.visible')
            cy.wait(500)
            cy.get('[role="option"]').contains('HAA').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'HAA')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@acaHaa')
            getIframeBody().find('p').contains('HAA Report').should('be.visible')
