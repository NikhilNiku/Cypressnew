...
context('UI',() => {
    ...
    describe('One VIN - ACA UI - THEFT',() => {
        beforeEach(() => {
            cy.logInUI({email: Cypress.env('username'), password: Cypress.env('password')})
            cy.selectACA()
        })
        it('requests a single vin theft',() => {
            cy.get('[name="VIN"]').type('JH4DC4433RS801008').blur()
            cy.get('[role="button"]').click()
            cy.get('[role="listbox"]').should('be.visible')
            cy.wait(500)
            cy.get('[role="option"]').contains('Theft').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'Theft')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@acaTheft')
            getIframeBody().find('p').contains('Theft Report').should('be.visible')
