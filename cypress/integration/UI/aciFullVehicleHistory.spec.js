...
context('UI',() => {
    ...
    describe('One VIN - ACI UI - FULL VEHICLE HISTORY REPORT',() => {
        beforeEach(() => {
        ...
        it('requests a single vin history report',() => {
            ...
            cy.wait(500)
            cy.get('[role="option"]').contains('Full Vehicle History ACI').click()
            cy.wait(500)
            cy.get('[role="button"]').should('contain', 'Full Vehicle History ACI')
            cy.wait(500)
            cy.get('button').contains('Submit').click()
            cy.wait('@aciFullHistory')
