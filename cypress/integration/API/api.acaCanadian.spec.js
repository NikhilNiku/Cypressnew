...
context('API', () => {
	describe('One VIN - ACA CANADIAN REPORT', () => {
		it('returns Canadian Report - Happy Path - ACA', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian',
					headers: {
				...
				.as('getCanadianReport');
			cy.get('@getCanadianReport').should((response) => {
				expect(response.status).to.eq(200);
                expect(response.body).to.have.property('level', 'CANADIAN');
		...
		it('returns Canadian Report - Lower Case Letters in VIN', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian',
		...
		it('fails to return Canadian Report - Invalid Token', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/histCanadian',
					headers: {
						'Content-Type': 'application.json',
						'Accept-Encoding': 'application.gzip',
						'x-autotec-id': 'ACDV0036',
		...
		it('fails to return Canadian Report - Invalid Request', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian',
	...
	describe('Ten VINs - CANADIAN REPORT', () => {
		it('returns 10 canadian reports', () => {
			2;
			cy
				.request({
					method: 'POST',
