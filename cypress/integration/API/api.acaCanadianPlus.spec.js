...
context('API', () => {
	describe('One VIN - ACA CANADIAN PLUS REPORT', () => {
		it('returns Canadian Plus Report - Happy Path - ACA', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian/plus',
					headers: {
				...
				.as('getCanadianPlusReport');
			cy.get('@getCanadianPlusReport').should((response) => {
				expect(response.status).to.eq(200);
                expect(response.body).to.have.property('level', 'CANADIAN_PLUS');
                ...
                expect(response.body.vehicles[0].history.historyRecords).to.have.property('length');
		...
		it('returns Canadian Plus Report - Lower Case Letters in VIN', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian/plus',
		...
		it('fails to return Canadian Plus Report - Invalid Token', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian/plus',
		...
		it('fails to return Canadian Plus Report - Invalid Request', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/canadian/plus',
	...
	describe('Ten VINs - CANADIAN PLUS REPORT', () => {
		it('returns 10 canadian plus reports', () => {
			2;
			cy
				.request({
					method: 'POST',
