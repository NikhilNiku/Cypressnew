...
context('API', () => {
	describe('One VIN - FULL HISTORY REPORT', () => {
		it('returns Full History Report - Happy Path - ACA', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('acaApi') + 'vehicle/history/full',
					headers: {
			...
			cy.get('@getFullHistoryReport').should((response) => {
				...
				expect(response.body.vehicles[0]).to.not.have.property('vehicleOwnerHistory');
			});
		});
