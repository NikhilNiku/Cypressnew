...
context('API', () => {
	describe('One VIN - Risk Plus', () => {
		it('returns Risk Plus - Happy Path - ACI', () => {
			cy
				.request({
					method: 'POST',
					url: Cypress.env('aciApi') + 'vehicle/risk/plus',
					headers: {
			...
			cy.get('@getRisk').should((response) => {
				...
				expect(response.body.riskResponse).to.have.property('type', 'Default');
				expect(response.body.riskResponse).to.have.property('vehicleRiskResults');
				expect(response.body.riskResponse.vehicleRiskResults[0]).to.have.property('vin');
				expect(response.body.riskResponse.vehicleRiskResults[0]).to.have.property('effectiveEndDate');
				expect(response.body.riskResponse.vehicleRiskResults[0]).to.have.property('referenceNumber');
				expect(response.body.riskResponse.vehicleRiskResults[0]).to.have.property('results');
