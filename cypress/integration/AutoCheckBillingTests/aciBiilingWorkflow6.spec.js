...
context('UI', () => {
    ...
    describe('ACI Billing Workflow 6 - Support for Tiered Volumn Rate based on Group Tiered Volume with Monthly Minimum Invoice', () => {
        var login = false;
        var workflowName = 'test workflow for ACI billing rule 6';
        ...
        beforeEach(() => {
            cy.loginToAutotecUI({ //Login to autotec dev url 
                email: Cypress.env('usernameaci6'),
                password: Cypress.env('passwordaci6')
            })
            cy.selectLocation();
        })
